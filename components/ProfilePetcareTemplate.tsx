...
import {
  ...
  FaCalendarAlt, FaLink, FaGift, FaConciergeBell, FaMotorcycle, FaPaw,
...
interface ProfilePetcareTemplateProps {
  profile: any;
  links?: any[];
...
const DEFAULT_SERVICES = [
  ...
  'Peluquería Canina / Felina', 'Evaluación Quirúrgica', 'Profilaxis Dental',
];

export const ProfilePetcareTemplate: React.FC<ProfilePetcareTemplateProps> = ({
  profile,
  links = [],
  onLinkClick,
  currentUser,
  ...
  // Lo que se dibuja dentro del modal: resultados de la búsqueda (de todas las
  // categorías) o el contenido de la categoría elegida.
  ...
  const accent = (profile?.buttonColor && String(profile.buttonColor).trim()) || '#0ea5a4';
  const clubName = (profile?.clubName && String(profile.clubName).trim()) || 'Club Pet VIP';
  const clubTagline = (profile?.clubTagline && String(profile.clubTagline).trim()) || 'Acumula sellos en cada visita y desbloquea baños, profilaxis y beneficios exclusivos para tu mascota.';
  ...
  const handlePlaceOrder = async () => {
    ...
    const vipNote = memberInfo?.found && memberDiscount > 0 ? `Miembro Pet VIP ${memberInfo.member_code} (-${memberDiscount}%)` : null;
  ...
  return (
    <div className={`min-h-screen relative overflow-x-clip ${profileStyleLayer(profile).className}`}
         style={{
           ...
           backgroundColor: profile?.backgroundColor || '#02100f',
      ...
      {profile?.coverImageURL ? (
        <div className="w-full h-52 sm:h-72 overflow-hidden relative">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#02100f] via-[#02100f]/20 to-black/30 pointer-events-none" />
        </div>
      ) : (
        <div className="w-full h-36 bg-gradient-to-br from-[#04241f] to-[#02100f] border-b border-white/5" />
      )}
      ...
      <div className="relative flex flex-col items-center px-4 -mt-16 pb-4 cinema-animate-in">
        ...
        {(profile?.profilePhotoURL || profile?.logoURL) && (
          <div className="relative">
            ...
            <img src={profile.profilePhotoURL || profile.logoURL} alt={profile.displayName}
              ...
              style={{ border: `2px solid ${accent}40`, boxShadow: '0 0 0 4px #02100f, 0 20px 60px rgba(0,0,0,0.9)' }} />
          </div>
        )}
        ...
        {socialProof && (
          <div className="mt-2 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[11px] font-bold text-[#e6fffb]/80">
        ...
        <div className="flex flex-wrap justify-center gap-2.5 mt-5">
          {profile?.phone && (
            <a href={`tel:${profile.phone}`} title="Llamar"
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:border-[#0ea5a4]/40 text-[#e6fffb]/70 hover:text-white transition-all hover:scale-105"><FaPhoneAlt size={14} /></a>
          )}
          {profile?.whatsapp && (
            <a href={`https://wa.me/${ecWaNumber(profile.whatsapp)}`} target="_blank" rel="noopener noreferrer" title="WhatsApp"
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:border-[#25D366]/40 text-[#e6fffb]/70 hover:text-[#25D366] transition-all hover:scale-105"><FaWhatsapp size={16} /></a>
          )}
          {profile?.contactEmail && (
            <a href={`mailto:${profile.contactEmail}`} title="Email"
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:border-[#0ea5a4]/40 text-[#e6fffb]/70 hover:text-white transition-all hover:scale-105"><FaEnvelope size={14} /></a>
          )}
          {setShowQRModal && (
            <button onClick={() => setShowQRModal(true)} title="Ver QR"
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:border-[#0ea5a4]/40 text-[#e6fffb]/70 hover:text-white transition-all hover:scale-105"><FaQrcode size={14} /></button>
          )}
          {downloadVCard && (
            <button onClick={downloadVCard} disabled={isVCardLoading} title="Guardar Contacto"
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:border-[#0ea5a4]/40 text-[#e6fffb]/70 hover:text-white transition-all hover:scale-105 disabled:opacity-50">📥</button>
          )}
        </div>
      </div>
        ...
        <div className="flex justify-center flex-wrap gap-2 mt-4 px-4">
          {profile.socialMedia.instagram && (
            <a href={`https://instagram.com/${profile.socialMedia.instagram.replace('@','')}`} target="_blank" rel="noopener noreferrer" title="Instagram"
              className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center border border-white/5 hover:border-[#0ea5a4]/30 text-[#e6fffb]/60 hover:text-white transition">
        ...
        <div className="flex flex-col items-center gap-1.5 px-4 mt-4 mb-1">
          {(Array.isArray(profile?.locations) && profile.locations.length > 0 ? profile.locations : [profile.location]).map((loc: string, i: number) => (
            <a key={i} href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(loc)}`} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-xs text-[#e6fffb]/60 hover:text-[#0ea5a4] transition-colors">
      ...
      <div className="px-4 max-w-md mx-auto mt-3">
        <button onClick={() => { setTrackCode(null); setShowTracker(true); }}
          className="w-full py-3 rounded-xl bg-white/5 border border-white/10 hover:border-[#0ea5a4]/40 text-[#e6fffb]/80 hover:text-white text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2">
        ...
        <div className="px-4 max-w-md mx-auto mt-4 space-y-2">
          {activeLinks.map((link: any) => (
            <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" onClick={() => onLinkClick?.(link.id)}
              className="flex items-center justify-center gap-2 w-full py-3 px-5 bg-white/5 border border-white/10 hover:border-[#0ea5a4]/30 rounded-xl text-xs font-bold uppercase tracking-wider text-[#e6fffb] hover:text-[#0ea5a4] transition-all hover:scale-[1.01]"><FaLink size={10} />{link.title}</a>
          ))}
        </div>
      )}
      ...
      <div className="px-4 max-w-md mx-auto mt-8">
        {sectionTitle(<FaPaw size={13} />, clubName)}
        <button onClick={() => setShowClub(true)}
          className="group relative w-full overflow-hidden rounded-2xl p-[1px] bg-gradient-to-r from-[#0ea5a4] via-[#5eead4] to-[#0ea5a4] active:scale-[0.99] transition-transform">
          <div className="relative rounded-2xl bg-[#06201d] px-5 py-4 flex items-center gap-4 text-left">
            <div className="w-11 h-11 rounded-xl bg-[#0ea5a4]/15 flex items-center justify-center text-xl shrink-0">🐾</div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-black text-white leading-tight">Afíliate al {clubName}</p>
              <p className="text-[11px] text-[#e6fffb]/60 mt-0.5 leading-snug">{clubTagline}</p>
            </div>
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 mb-3">
                {categoryNames.map((cat) => (
                  <button key={cat} onClick={() => setSelectedCat(cat)}
                    ...
                    className={`px-4 py-2 rounded-full text-[11px] font-black uppercase tracking-wider shrink-0 border transition-all ${selectedCat === cat ? 'text-white shadow-lg' : 'bg-[#0ea5a4]/5 border-[#0ea5a4]/10 text-gray-400 hover:text-white hover:border-[#0ea5a4]/25'}`}>{cat}</button>
                ))}
              </div>
            )}
              ...
              {(categorizedProducts[selectedCat] || []).slice(0, 12).map((product) => {
                ...
                return (
                  <div key={product.id} onClick={() => openDish(product)}
                    className={`snap-start shrink-0 w-40 rounded-2xl overflow-hidden border bg-white/[0.03] transition-all duration-300 ${agotado ? 'opacity-60 cursor-not-allowed border-white/5' : 'cursor-pointer border-[#0ea5a4]/10 hover:border-[#0ea5a4]/30 hover:-translate-y-0.5'}`}>
                    <div className="relative">
                      {product.imageURL ? (
                        <img src={product.imageURL} alt={product.name} className={`w-full h-28 object-cover ${agotado ? 'grayscale' : ''}`} />
                      ) : (
                        <div className="w-full h-28 bg-gradient-to-br from-[#04241f] to-[#031a16] flex items-center justify-center text-[#0ea5a4]/25"><FaPaw size={22} /></div>
                      )}
                    ...
                    <div className="p-3">
                      <h4 className="cnx-product-text font-bold text-xs text-[#e6fffb] truncate leading-snug">{product.cleanName}</h4>
      ...
      {profile?.showSchedule !== false && businessHours.length > 0 && (
        <div className="px-4 max-w-md mx-auto mt-8">
          <div className="bg-white/[0.03] border border-[#0ea5a4]/10 rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {businessHours.map((day: any, i: number) => (
                <div key={i} className="flex justify-between items-center text-xs py-1 border-b border-white/5 last:border-b-0">
                  <span className="text-[#e6fffb]/80 font-semibold capitalize">{day.day}</span>
      ...
      {sectionOn('testimonials') && isProOrUltra && Array.isArray(profile?.testimonials) && profile.testimonials.length > 0 && (
        <div className="px-4 max-w-md mx-auto mt-8">
          {sectionTitle(<FaStar size={12} />, 'Dueños que confían en nosotros')}
          <div className="space-y-3.5">
            {profile.testimonials.map((test: any) => (
              <div key={test.id} className="relative overflow-hidden rounded-2xl p-[1px] bg-gradient-to-br from-[#0ea5a4]/30 via-[#0ea5a4]/5 to-transparent">
                <div className="relative rounded-2xl bg-gradient-to-br from-[#06231f] to-[#041a16] p-5">
                  <span className="absolute top-2 right-4 text-5xl leading-none font-serif text-[#0ea5a4]/15 select-none pointer-events-none">”</span>
                  <div className="flex items-center gap-0.5 mb-2.5" style={{ color: accent }}>{Array.from({ length: 5 }).map((_, i) => <FaStar key={i} size={11} />)}</div>
                  <p className="cnx-testimonial-text italic text-[13px] text-[#e6fffb]/90 leading-relaxed relative z-10">"{test.quote}"</p>
                  <div className="flex items-center gap-3 mt-4 pt-3.5 border-t border-[#0ea5a4]/10">
                    {test.authorImageURL ? (
                      <img src={test.authorImageURL} alt={test.author} className="w-9 h-9 rounded-full object-cover border-2 border-[#0ea5a4]/40" />
                    ) : (
                      <div className="w-9 h-9 rounded-full bg-[#0ea5a4]/15 border border-[#0ea5a4]/30 flex items-center justify-center font-black text-sm shrink-0" style={{ color: accent }}>{String(test.author || '?').charAt(0).toUpperCase()}</div>
                    )}
                    <div className="min-w-0"><p className="font-black text-xs text-white truncate">{test.author}</p>{test.authorTitle && <p className="text-[10px] text-[#e6fffb]/40 truncate">{test.authorTitle}</p>}</div>
      ...
      {((Array.isArray(profile?.digitalFiles) && profile.digitalFiles.length > 0) || (Array.isArray(profile?.faqs) && profile.faqs.length > 0)) && (
        <div className="px-4 max-w-md mx-auto mt-8">
          {sectionTitle(<FaBookMedical size={13} />, 'Centro de Propietarios')}
          <p className="text-[11px] text-gray-500 mb-3 leading-snug">Guías de cuidado y respuestas rápidas para que cuides a tu mascota sin esperas.</p>
        </div>
      )}
      ...
      {activeOrderModal && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-black/85 backdrop-blur-sm" onClick={() => setActiveOrderModal(null)}>
          <div className="bg-[#06201d] border border-[#0ea5a4]/25 w-full max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl relative max-h-[85vh] sm:max-h-[90vh] flex flex-col overflow-hidden text-white" onClick={(e) => e.stopPropagation()}>
            <div className="relative shrink-0">
              ...
              {activeOrderModal.imageURL && <img src={activeOrderModal.imageURL} alt={activeOrderModal.cleanName} className="w-full h-48 sm:h-56 object-cover border-b border-[#0ea5a4]/15" />}
            </div>
            <div className="p-5 overflow-y-auto flex-1">
              ...
              <div className="mt-6 pt-5 border-t border-[#0ea5a4]/15 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setOrderType('mesa')}
                    className={`py-3 px-3 rounded-xl text-xs font-bold transition-all border flex flex-col items-center gap-1.5 ${orderType === 'mesa' ? 'bg-[#0ea5a4]/15 border-[#0ea5a4]/40 text-[#e6fffb]' : 'bg-black/30 border-white/5 text-gray-400'}`}><FaConciergeBell size={16} /> Retiro en Clínica</button>
                  <button type="button" onClick={() => setOrderType('delivery')}
                    className={`py-3 px-3 rounded-xl text-xs font-bold transition-all border flex flex-col items-center gap-1.5 ${orderType === 'delivery' ? 'bg-[#0ea5a4]/15 border-[#0ea5a4]/40 text-[#e6fffb]' : 'bg-black/30 border-white/5 text-gray-400'}`}><FaMotorcycle size={16} /> Delivery Local</button>
                </div>
                {orderType === 'mesa' ? (
                  <div>
                    ...
                    <input type="text" value={mesaNumber} onChange={(e) => setMesaNumber(e.target.value)} placeholder="Ej: Ana Pérez"
                      className="w-full p-3 rounded-xl bg-black/40 border border-[#0ea5a4]/20 text-[#e6fffb] placeholder-gray-600 focus:outline-none focus:border-[#0ea5a4] text-sm" />
                  </div>
                ) : (
                  <div className="space-y-2.5">
                    <input type="text" value={custName} onChange={(e) => setCustName(e.target.value)} placeholder="Tu nombre *" className="w-full p-3 rounded-xl bg-black/40 border border-[#0ea5a4]/20 text-[#e6fffb] placeholder-gray-600 focus:outline-none focus:border-[#0ea5a4] text-sm" />
                    <input type="tel" value={custPhone} onChange={(e) => setCustPhone(e.target.value)} placeholder="Teléfono / WhatsApp *" className="w-full p-3 rounded-xl bg-black/40 border border-[#0ea5a4]/20 text-[#e6fffb] placeholder-gray-600 focus:outline-none focus:border-[#0ea5a4] text-sm" />
                    <textarea value={custAddress} onChange={(e) => setCustAddress(e.target.value)} placeholder="Dirección de entrega *" rows={2} className="w-full p-3 rounded-xl bg-black/40 border border-[#0ea5a4]/20 text-[#e6fffb] placeholder-gray-600 focus:outline-none focus:border-[#0ea5a4] text-sm resize-none" />
                  </div>
                )}
                <div className="flex items-center justify-between bg-black/20 p-3 rounded-xl border border-[#0ea5a4]/10">
                  <span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Cantidad</span>
                  <div className="flex items-center gap-3">
                    <button type="button" onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-8 h-8 rounded-lg bg-white/5 border border-[#0ea5a4]/10 flex items-center justify-center text-sm font-black active:bg-white/10">-</button>
                    <span className="font-mono font-bold text-sm w-4 text-center">{quantity}</span>
                    <button type="button" onClick={() => setQuantity(q => q + 1)} className="w-8 h-8 rounded-lg bg-white/5 border border-[#0ea5a4]/10 flex items-center justify-center text-sm font-black active:bg-white/10">+</button>
                  </div>
                </div>
                <div className="pt-4 border-t border-[#0ea5a4]/15">
                  <label className="text-[10px] font-black uppercase tracking-wider mb-2 flex items-center gap-1.5" style={{ color: accent }}><FaStar size={10} /> ¿Eres miembro Pet VIP? Aplica tu código</label>
                  <div className="flex gap-2">
                    <input type="text" value={memberCode} onChange={(e) => { setMemberCode(e.target.value); setMemberChecked(false); setMemberInfo(null); }} placeholder="Tu código de miembro"
                      className="flex-1 p-3 rounded-xl bg-black/40 border border-[#0ea5a4]/20 text-[#e6fffb] placeholder-gray-600 focus:outline-none focus:border-[#0ea5a4] text-sm uppercase tracking-widest font-mono" />
                    <button type="button" onClick={applyMemberCode} disabled={!memberCode.trim() || checkingMember} className="px-4 rounded-xl bg-[#0ea5a4]/15 border border-[#0ea5a4]/30 text-xs font-black uppercase tracking-wider hover:bg-[#0ea5a4]/25 disabled:opacity-40 transition" style={{ color: accent }}>{checkingMember ? '...' : 'Aplicar'}</button>
                  </div>
                  ...
                  {memberChecked && !memberInfo?.found && (
                    <div className="mt-2 p-2.5 rounded-lg bg-white/5 border border-[#0ea5a4]/15 text-xs text-gray-300 flex items-center justify-between gap-2"><span>No encontramos ese código.</span><button type="button" onClick={() => { setActiveOrderModal(null); setShowClub(true); }} className="font-black uppercase tracking-wider text-[10px] shrink-0" style={{ color: accent }}>Afíliate →</button></div>
                  )}
                </div>
              </div>
            </div>
            <div className="p-4 border-t border-[#0ea5a4]/15 shrink-0 bg-black/30">
              {memberDiscount > 0 && (
      ...
      {successOrder && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm" onClick={() => setSuccessOrder(null)}>
          <div className="bg-[#06201d] border-2 border-green-500/30 w-full max-w-md rounded-2xl shadow-2xl relative p-6 flex flex-col items-center text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="my-6 p-4 rounded-xl bg-black/40 border border-[#0ea5a4]/20 w-full">
            ...
            <button type="button" onClick={() => setInvoiceCode(successOrder.verification_code)} className="mt-2 w-full py-3 bg-white/5 hover:bg-white/10 border border-[#0ea5a4]/20 text-[#e6fffb] rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2">🧾 ¿Necesitas factura?</button>
            ...
            <button type="button" onClick={() => setSuccessOrder(null)} className="mt-2 w-full py-3 bg-white/5 hover:bg-white/10 border border-[#0ea5a4]/20 text-[#e6fffb] rounded-xl text-xs font-black uppercase tracking-wider transition-all">Cerrar</button>
          </div>
        </div>
      )}
      ...
      {showFullMenu && (
        <FullCatalogView
          ...
          title="PetShop y Farmacia"
          searchPlaceholder="Buscar (ej. Amoxicilina, Royal Canin, Bravecto)"
...
const RES_META: Record<string, { label: string; emoji: string; cls: string }> = {
  ...
  sentada: { label: '¡Tu mascota está en consulta!', emoji: '🐾', cls: 'bg-blue-500/10 border-blue-500/25 text-blue-300' },
...
const OrderTracker: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[118] flex items-end sm:items-center justify-center p-4 bg-black/85 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm bg-[#06201d] rounded-2xl border border-[#0ea5a4]/30 shadow-2xl p-6 relative" onClick={(e) => e.stopPropagation()} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        {!tracking ? (
          <form onSubmit={(e) => { e.preventDefault(); if (code.trim()) setTracking(true); }} className="space-y-3">
            ...
            <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Código (ej. A3F9C)" className="w-full bg-black/40 border border-[#0ea5a4]/20 text-white text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-[#0ea5a4] placeholder-gray-600 uppercase tracking-widest font-mono" />
          ...
          (() => {
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="rounded-xl bg-black/30 border border-[#0ea5a4]/15 divide-y divide-white/5">
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5"><span className="text-xs text-gray-400">Mascotas</span><span className="text-xs font-bold text-white">{data.party_size}</span></div>
                  <div className="flex items-center justify-between px-4 py-2.5"><span className="text-xs text-gray-400">Box asignado</span><span className="text-xs font-bold text-white">{data.table_number ? `Box ${data.table_number}` : 'Por asignar'}</span></div>
                </div>
                <div className={`p-2.5 rounded-xl border text-xs font-bold text-center ${rm.cls}`}>
                  ...
                  {data.status === 'sentada' && '🐾 Tu mascota está en consulta. ¡Gracias por tu confianza!'}
                ...
                {Array.isArray(data.preorder_items) && data.preorder_items.length > 0 && (
                  <div className="p-3 rounded-xl bg-[#0ea5a4]/10 border border-[#0ea5a4]/20"><p className="text-[10px] font-black uppercase tracking-widest mb-1.5" style={{ color: accent }}>Tu pre-pedido</p><div className="space-y-1">{data.preorder_items.map((it: any, i: number) => (<div key={i} className="flex justify-between text-xs text-[#e6fffb]/85"><span>{it.qty}x {it.name}</span>{it.price && <span className="font-mono" style={{ color: accent }}>{it.price}</span>}</div>))}</div></div>
                )}
          ...
          <div className="space-y-4">
            ...
            {onInvoice && <button onClick={() => onInvoice(data.code)} className="w-full py-2.5 rounded-lg bg-white/5 border border-[#0ea5a4]/20 text-[#e6fffb] text-xs font-black uppercase tracking-wider hover:bg-white/10 transition">🧾 ¿Necesitas factura?</button>}
            <p className="text-[10px] text-gray-500 text-center">Esta pantalla se actualiza sola cada pocos segundos.</p>
          </div>
        )}
      </div>
...
const InvoiceModal: React.FC<{ restaurantId: string | null; code: string; isMobilePreview: boolean; accent: string; onClose: () => void; }> = ({ restaurantId, code, isMobilePreview, accent, onClose }) => {
  ...
  const inp = "w-full bg-black/40 border border-[#0ea5a4]/20 text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#0ea5a4] placeholder-gray-600";
  return (
    <div className="fixed inset-0 z-[122] flex items-end sm:items-center justify-center p-4 bg-black/85 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm bg-[#06201d] rounded-2xl border border-[#0ea5a4]/30 shadow-2xl p-6 relative max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-white transition"><FaTimes size={16} /></button>
        ...
        {done ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="flex gap-2">
              ...
              <button type="button" onClick={prefillFromMember} disabled={!memberCode.trim() || loadingMember} className="px-4 rounded-xl bg-[#0ea5a4]/15 border border-[#0ea5a4]/30 text-xs font-black uppercase hover:bg-[#0ea5a4]/25 disabled:opacity-40 transition shrink-0" style={{ color: accent }}>{loadingMember ? '...' : 'Usar'}</button>
            </div>
...
// ════════════════════════════════════════════════════════════════════════
// Carnet Pet VIP: consulta de membresía + sellos
// ════════════════════════════════════════════════════════════════════════
const MembershipLookup: React.FC<{ restaurantId: string | null; isMobilePreview: boolean; clubName?: string; accent: string; stampsReward?: string; onJoinClick: () => void; }> = ({ restaurantId, isMobilePreview, clubName = 'Club Pet VIP', accent, stampsReward, onJoinClick }) => {
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.found) {
        ...
        try {
          const { data: recs } = await supabase.rpc('get_petcare_records', { p_restaurant_id: restaurantId, p_member_code: data.member_code });
  ...
  const StampRow: React.FC<{ visits: number }> = ({ visits }) => {
    ...
    return (
      <div className="mt-3 p-3 rounded-xl bg-black/30 border border-[#0ea5a4]/20">
        <div className="flex items-center justify-between mb-2"><p className="text-[10px] font-black uppercase tracking-widest" style={{ color: accent }}>🐾 Carnet de sellos</p><span className="text-[10px] text-gray-500">{filled}/{cycle}</span></div>
        <div className="flex items-center justify-between gap-1.5">
          {Array.from({ length: cycle }).map((_, i) => (
            <div key={i} className="flex-1 aspect-square rounded-lg flex items-center justify-center text-sm border" style={i < filled ? { backgroundColor: `${accent}22`, borderColor: `${accent}66`, color: accent } : { background: 'rgba(0,0,0,0.3)', borderColor: 'rgba(255,255,255,0.08)', color: '#4b5563' }}>{i === cycle - 1 ? '🎁' : <FaPaw size={12} />}</div>
          ))}
        </div>
        {stampsReward && String(stampsReward).trim() && <p className="text-[11px] text-[#e6fffb]/80 mt-2 leading-snug"><b style={{ color: accent }}>Premio:</b> {stampsReward}</p>}
      </div>
    );
  };
  ...
  if (result?.found || result?.member_code) {
    return (
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#052320] via-[#041a16] to-[#06302b] border border-[#0ea5a4]/30 p-5 shadow-2xl">
        ...
        <div className="flex justify-between items-start mb-3">
          ...
          {result.is_vip && <div className="px-2.5 py-1 rounded-lg bg-[#0ea5a4]/15 border border-[#0ea5a4]/30 text-[9px] font-black uppercase tracking-widest" style={{ color: accent }}>⭐ Pet VIP</div>}
        </div>
        <div className="p-4 rounded-xl bg-black/40 border border-[#0ea5a4]/20 text-center">
        ...
        {result.benefits && String(result.benefits).trim() && (
          <div className="mt-3 p-3 rounded-xl bg-[#0ea5a4]/10 border border-[#0ea5a4]/20"><p className="text-[10px] font-black uppercase tracking-widest mb-1" style={{ color: accent }}>🎁 Tus beneficios</p><p className="text-xs text-[#e6fffb]/85 leading-snug whitespace-pre-wrap">{result.benefits}</p></div>
        )}
            ...
            {showMedHistory && (
              <div className="mt-2 space-y-2 max-h-64 overflow-y-auto pr-1">
                {medRecords.map((rec: any, i: number) => (
                  <div key={rec.id ?? i} className="bg-black/40 border border-[#0ea5a4]/15 rounded-xl p-3 text-xs">
                    ...
                    {rec.pet_name && <div className="text-[#0ea5a4]/80 text-[10px] mb-1">🐾 {rec.pet_name}</div>}
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#052320] via-[#041a16] to-[#06302b] border border-[#0ea5a4]/30 p-5 shadow-2xl">
      ...
      <div className="flex justify-between items-start mb-4">
        <div><div className="text-[10px] uppercase font-bold tracking-widest" style={{ color: `${accent}cc` }}>Expediente de Salud</div><h3 className="text-base font-bold text-white mt-0.5">Carnet de mi mascota</h3></div>
        <div className="px-2.5 py-1 rounded-lg bg-[#0ea5a4]/10 border border-[#0ea5a4]/20 text-[9px] font-black uppercase tracking-widest" style={{ color: accent }}>Pet VIP</div>
      </div>
      <p className="text-xs text-gray-400 mb-3 leading-relaxed">¿Ya eres miembro? Ingresa tu código, email o teléfono para ver tu carnet, tus sellos y beneficios.</p>
      <form onSubmit={handleLookup} className="space-y-2.5">
        <input type="text" value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="Tu código, email o teléfono" className="w-full bg-black/40 border border-[#0ea5a4]/20 text-white text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-[#0ea5a4] placeholder-gray-600" />
...
// ════════════════════════════════════════════════════════════════════════
// Afiliación Pet VIP (captura de leads → join_gastro_club)
// ════════════════════════════════════════════════════════════════════════
const ClubPetModal: React.FC<{ restaurantId: string | null; isMobilePreview: boolean; clubName?: string; clubTagline?: string; accent: string; onClose: () => void; }> = ({ restaurantId, isMobilePreview, clubName = 'Club Pet VIP', clubTagline = 'Beneficios exclusivos para los engreídos de la casa', accent, onClose }) => {
  ...
  const inp = "w-full bg-black/40 border border-[#0ea5a4]/20 text-white text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-[#0ea5a4] placeholder-gray-600";
  return (
    <div className="fixed inset-0 z-[115] flex items-end sm:items-center justify-center p-4 bg-black/85 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm bg-[#06201d] rounded-2xl border border-[#0ea5a4]/30 shadow-2xl p-6 relative" onClick={(e) => e.stopPropagation()} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        {done ? (
          ...
          <>
            <div className="flex items-center gap-3 mb-4"><div className="w-11 h-11 rounded-xl bg-[#0ea5a4]/15 flex items-center justify-center text-xl shrink-0">🐾</div><div><h3 className="text-base font-black text-white leading-tight">Afíliate al {clubName}</h3><p className="text-[11px] text-gray-400 mt-0.5">{clubTagline}</p></div></div>
            <form onSubmit={handleSubmit} className="space-y-3">
              <input required type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Tu nombre *" className={inp} />
              <input type="text" value={petName} onChange={(e) => setPetName(e.target.value)} placeholder="Nombre de tu mascota" className={inp} />
            ...
            <p className="text-[10px] text-gray-600 text-center mt-3">Sin spam. Solo recordatorios de vacunas y novedades.</p>
          </>
        )}
      </div>
    </div>
...
const ReserveModal: React.FC<{
  ...
  const [petName, setPetName] = useState(''); const [species, setSpecies] = useState('Perro'); const [breed, setBreed] = useState(''); const [age, setAge] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const patientNote = `Servicio: ${service}. Paciente: ${petName.trim()} (${species}${breed.trim() ? ', ' + breed.trim() : ''}${age.trim() ? ', ' + age.trim() : ''}).${notes.trim() ? ' Motivo: ' + notes.trim() : ''}`;
      ...
      <div className="w-full max-w-sm bg-[#06201d] rounded-2xl border border-[#0ea5a4]/30 shadow-2xl p-6 relative max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        {result ? (
          <div className="text-center py-4 space-y-3">
            ...
            {result.code && (<div className="my-2 p-4 rounded-xl bg-black/40 border border-[#0ea5a4]/30"><div className="text-[9px] uppercase font-bold tracking-widest text-gray-500 mb-1">Código de cita</div><div className="text-3xl font-mono font-black tracking-[0.3em]" style={{ color: accent }}>{result.code}</div></div>)}
          ...
          <>
            <div className="flex items-center gap-3 mb-4"><div className="w-11 h-11 rounded-xl bg-[#0ea5a4]/15 flex items-center justify-center text-xl shrink-0">🩺</div><div><h3 className="text-base font-black text-white leading-tight">Agendar una cita</h3><p className="text-[11px] text-gray-400 mt-0.5">Datos del paciente + fecha y hora.</p></div></div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="rounded-xl border border-[#0ea5a4]/15 bg-black/20 p-3 space-y-2.5">
                <p className="text-[11px] font-black uppercase tracking-widest flex items-center gap-1.5" style={{ color: accent }}><FaPaw size={11} /> Datos del paciente</p>
                <input required value={petName} onChange={(e) => setPetName(e.target.value)} placeholder="Nombre de la mascota *" className={inp} />
                <div className="grid grid-cols-2 gap-2">
                  <select value={species} onChange={(e) => setSpecies(e.target.value)} className={inp}><option>Perro</option><option>Gato</option><option>Ave</option><option>Conejo</option><option>Otro</option></select>
                  <input value={age} onChange={(e) => setAge(e.target.value)} placeholder="Edad (ej. 3 años)" className={inp} />
                </div>
                <input value={breed} onChange={(e) => setBreed(e.target.value)} placeholder="Raza (opcional)" className={inp} />
              </div>
              ...
              <div>
                <label className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1">Mascotas</label>
                <div className="flex items-center gap-3 bg-black/40 border border-[#0ea5a4]/20 rounded-xl px-4 py-2.5">
                  <button type="button" onClick={() => setParty(p => Math.max(1, p - 1))} className="w-8 h-8 rounded-lg bg-white/5 border border-[#0ea5a4]/10 flex items-center justify-center text-sm font-black text-white">−</button>
                  <span className="font-mono font-bold text-white flex-1 text-center">{party}</span>
                  <button type="button" onClick={() => setParty(p => Math.min(maxParty, p + 1))} className="w-8 h-8 rounded-lg bg-white/5 border border-[#0ea5a4]/10 flex items-center justify-center text-sm font-black text-white">+</button>
                </div>
                <p className="text-[10px] text-gray-600 mt-1">Máximo {maxParty} mascotas por cita.</p>
              </div>
              ...
              <div>
                <label className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1">¿Eres miembro Pet VIP? (autocompleta tus datos)</label>
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={lookupMember} disabled={!memberCode.trim() || memberLookupLoading} className="shrink-0 px-3.5 rounded-xl bg-[#0ea5a4]/15 border border-[#0ea5a4]/30 text-xs font-black uppercase disabled:opacity-40 transition" style={{ color: accent }}>{memberLookupLoading ? '...' : 'Buscar'}</button>
                </div>
                {memberLookupInfo?.found && (
                  <div className="mt-1.5 p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs font-bold flex items-center gap-1.5">✅ {memberLookupInfo.name || 'Miembro encontrado'} — datos completados</div>
                )}
                {memberLookupInfo && !memberLookupInfo.found && (
                  <div className="mt-1.5 p-2 rounded-lg bg-white/5 border border-[#0ea5a4]/15 text-xs text-gray-400">No encontramos esa membresía. Puedes continuar igual.</div>
                )}
              </div>
              {errorMsg && <div className="p-2.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-300 text-xs font-bold text-center">{errorMsg}</div>}
