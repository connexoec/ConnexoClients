...
import type { RealtyClient } from '../../types';
import {
  FaCrown, FaUserPlus, FaTrash, FaStar, FaPhone, FaEnvelope,
  FaWhatsapp, FaUsers, FaPercent, FaSave, FaGift,
} from 'react-icons/fa';
...
export const RealtyLoyaltyTab: React.FC<Props> = ({ user }) => {
  const planOptions = useSubscriptionOptions(user?.id, "none");
  const [clients, setClients] = useState<RealtyClient[]>([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState<'all' | 'vip' | 'club'>('all');
  ...
  const fetchClients = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('realty_clients').select('*').eq('agency_profile_id', user.id)
        .order('is_vip', { ascending: false }).order('created_at', { ascending: false });
      if (error) throw error;
      const list = (data ?? []) as RealtyClient[];
      if (initialisedRef.current) {
        const fresh = list.filter(c => !knownIdsRef.current.has(c.id));
        if (fresh.length > 0) {
          const n = fresh[0]; playPing();
          setAlertMsg(`🎉 Nuevo miembro: ${n.name || n.phone || n.email || 'cliente'}${n.source === 'club' ? ' (Club)' : ''}`);
      ...
      console.error('RealtyLoyaltyTab fetch error:', err?.message ?? err);
  ...
  useEffect(() => {
    if (!user?.id) return;
    fetchClients();
    const channel = supabase.channel(`realty_clients_${user.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'realty_clients', filter: `agency_profile_id=eq.${user.id}` }, () => fetchClients())
      .subscribe();
  ...
  const handleAdd = async () => {
    if (!newClient.name.trim() && !newClient.phone.trim()) { alert('Indica al menos el nombre o el teléfono.'); return; }
    try {
      const { error } = await supabase.from('realty_clients').insert({
        ...
        source: 'manual', member_code: `R-${Math.random().toString(36).slice(2, 6).toUpperCase()}`,
      });
      if (error) throw error;
  ...
  const toggleVip = async (c: RealtyClient) => {
    try {
      const { error } = await supabase.from('realty_clients').update({ is_vip: !c.is_vip }).eq('id', c.id);
      if (error) throw error;
      setClients(prev => prev.map(x => x.id === c.id ? { ...x, is_vip: !c.is_vip } : x));
    } catch (err: any) { alert(`Error: ${err?.message ?? err}`); }
  };

  const setDraft = (id: string, patch: Partial<{ discount_percent: number; notes: string; benefits: string }>, base: RealtyClient) => {
    setDrafts(prev => ({ ...prev, [id]: { discount_percent: base.discount_percent, notes: base.notes ?? '', benefits: base.benefits ?? '', ...prev[id], ...patch } }));
  };

  const saveDraft = async (c: RealtyClient) => {
    const draft = drafts[c.id] ?? { discount_percent: c.discount_percent, notes: c.notes ?? '', benefits: c.benefits ?? '' };
    setSavingId(c.id);
    try {
      const { error } = await supabase.from('realty_clients').update({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('realty_clients').delete().eq('id', id);
      if (error) throw error;
      setClients(prev => prev.filter(x => x.id !== id));
    } catch (err: any) { alert(`Error al eliminar: ${err?.message ?? err}`); }
  };

  const filtered = clients.filter(c => filter === 'all' ? true : filter === 'vip' ? c.is_vip : c.source === 'club');
  const vipCount = clients.filter(c => c.is_vip).length;
  const clubCount = clients.filter(c => c.source === 'club').length;
  const inpCls = "bg-black/40 border border-[#ff6600]/20 text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#ff6600] placeholder-gray-600 w-full";
  ...
  return (
    <div className="animate-fade-in-up space-y-6 max-w-4xl mx-auto px-4 md:px-6 pb-32" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-5" style={{ borderColor: `${ACCENT}26` }}>
        <div>
          <h2 className="text-2xl font-black text-white flex items-center gap-2 italic" style={{ fontFamily: TITLE_FONT }}>
            <FaCrown style={{ color: ACCENT }} /> Club de Inversionistas
          </h2>
          <p className="text-sm text-white/40 mt-1">Tus clientes VIP, recurrentes y del club — en un solo lugar.</p>
        </div>
        <button onClick={() => setAdding(a => !a)} className="flex items-center gap-2 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-white rounded-xl italic" style={{ background: ACCENT, fontFamily: TITLE_FONT }}>
      ...
      {adding && (
        <div className="bg-white/5 border border-[#ff6600]/20 rounded-2xl p-5 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <input value={newClient.notes} onChange={e => setNewClient(p => ({ ...p, notes: e.target.value }))} placeholder="Notas (ej. busca casa en Cumbayá)" className={inpCls} />
          </div>
          <button onClick={handleAdd} className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-white text-xs font-black uppercase tracking-wider" style={{ background: ACCENT }}>Guardar</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Club', value: clubCount, icon: <FaStar /> },
        ].map(k => (
      ...
      <div className="flex flex-wrap gap-2">
        {([['all', 'Todos'], ['vip', 'VIP'], ['club', 'Club']] as const).map(([f, label]) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${filter === f ? 'text-white' : 'bg-white/5 text-white/50 hover:text-white border border-white/10'}`}
            style={filter === f ? { background: ACCENT } : undefined}>{label}</button>
        ))}
      </div>

      {filtered.length === 0 ? (
        <div className="text-center py-16 text-white/30"><p className="text-4xl mb-3">👥</p><p className="text-sm">Aún no hay clientes. Cuando alguien se una al club o solicite una cotización, aparecerá aquí.</p></div>
      ) : (
        <div className="space-y-3">
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} className={`rounded-2xl border p-4 transition-all ${c.is_vip ? 'bg-gradient-to-br from-[#ff6600]/10 to-transparent border-[#ff6600]/30' : 'bg-white/[0.03] border-white/8'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-[11px] text-white/50">
                      ...
                      <span className="flex items-center gap-1"><FaStar size={9} style={{ color: `${ACCENT}99` }} /> {c.visits} interaccion{c.visits !== 1 ? 'es' : ''}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button onClick={() => toggleVip(c)} className={`w-8 h-8 rounded-lg flex items-center justify-center transition border ${c.is_vip ? 'text-white' : 'bg-white/5 border-white/10 text-white/40'}`} style={c.is_vip ? { background: `${ACCENT}33`, borderColor: `${ACCENT}66`, color: ACCENT_LIGHT } : undefined} title={c.is_vip ? 'Quitar VIP' : 'Marcar VIP'}><FaCrown size={12} /></button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[140px_1fr_auto] gap-2 mt-3 pt-3 border-t border-white/5 items-center">
                  ...
                  <input value={draft ? draft.notes : (c.notes ?? '')} onChange={e => setDraft(c.id, { notes: e.target.value }, c)}
                    placeholder="Notas internas (ej. presupuesto, zona de interés…)" className="bg-black/30 border border-white/5 text-white text-sm rounded-lg px-3 py-2 focus:outline-none placeholder-gray-600" />
                  <button onClick={() => saveDraft(c)} disabled={!dirty || savingId === c.id}
                    className="flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg disabled:opacity-30 text-white text-xs font-black uppercase tracking-wider transition-all" style={{ background: ACCENT }}>
                ...
                <div className="mt-2">
                  <input value={draft ? draft.benefits : (c.benefits ?? '')} onChange={e => setDraft(c.id, { benefits: e.target.value }, c)}
                    placeholder="🎁 Beneficios que ve el cliente (ej. Avalúo gratis, prioridad en preventas…)"
                ...
                <MemberPlanSection client={c as any} system="realty" ownerId={user?.id}
...
export default RealtyLoyaltyTab;
